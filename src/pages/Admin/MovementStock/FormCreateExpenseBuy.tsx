...
import type { ExpenseBuyCreate, ItemExpenseBuy, PaymentMethod } from "@/hooks/admin/MovementStock/movementStockType";
...
const FormCreateExpenseBuy = () => {
  ...
  const handleRemoveItem = (index: number) => {
    ...
    const stillUsed = items.some((item, i) => i !== index && item.product_id === removedItem.product_id);
    if (!stillUsed) {
      const newMap = { ...productsMap };
      delete newMap[removedItem.product_id];
  ...
  const onSubmit = (data: ExpenseBuyCreate) => {
    ...
    createExpenseBuy(expenseBuyData);
  };
            ...
            <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-red-300/80 text-xs">
                  {createError.message || "Ha ocurrido un error inesperado"}
                </p>
              </div>
            </div>
          )}
